...
public class PrincipalDetails implements UserDetails, OAuth2User {
    ...
    public PrincipalDetails(User user, Map<String , Object> attributes){
        this.user = user;  //  이때 User 정보는, 인증 직후 provider 로 부터 받은 attributes 를 토대로 생성하게 된다.
...
// 현재 로그인한 사용자의 권한정보가 필요할때마다 호출된다. 혹은 필요할때마다 직접 호출해 사용할수도 있다
    ...
    //  -----------------------------------------------
    // OAuth2User 를 implement 하면 구현 할 메소드

    private Map<String , Object> attributes;  //  OAuth2user 의 getAttributes() 값

    @Override
    public String getName() {
        return null;  // 사용하지 않을예정
    }
    @Override
    public Map<String, Object> getAttributes() {
        return attributes;  //  어디서 받아올까? -> 생성자에서 받지요
    }
}
