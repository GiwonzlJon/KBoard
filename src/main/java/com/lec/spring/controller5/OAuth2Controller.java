...
public class OAuth2Controller {
    ...
    public String kakaoCallBack(String code) {  //  Kakao가 보내준 code 값 받아오기
        //------------------------------------------------------------------
        // ■ code 값 확인
...
// Kakao 로그인 한 회원을 User 에 등록하기
    ...
    private User registerKakaoUser(KakaoProfile profile) {
        //  새로이 가입시킬 username 을 생성(unique해야 한당)
...
// 이미 가입한 회원인지, 혹은 비가입자인지 체크하여야 한다
        ...
        if (user == null) {  //  미가입자 인 경우 회원가입 진해
    ...
    private KakaoProfile kakaoUserInfo(String accessToken) {
        ...
        System.out.println("카카오 사용자 Profile 요청 읍답 : " + response);
        System.out.println("카카오 사용자 Profile 읍답 BODY : " + response.getBody());
    ...
    private KakaoOAuthToken kakaoAccessToken(String code) {
        ...
        //  위 header, body 를 담은 HttpEntity 생성
