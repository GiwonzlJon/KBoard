    ...
    $.ajax({
        ...
        success: function (data, status) {
            if (status == "success") {
                ...
                //  이벤트 설정
                //  댓글 목록을 불러오고 난 뒤에 삭제에 대한 이벤트 리스너 등록해야한다.
                addDelete();
...
function buildComment(result) {
    $("#cmt_cnt").text(result.count); // 댓글 총 갯수
    ...
    result.data.forEach(comment => {
        ...
        //  삭제버튼 여부 : 작성자 본인인 경우에만 삭제버튼 보이게 하기
...
//  댓글 삭제 버튼이 눌렸을때 해당댓글 삭제하는 동작을 이벤트 핸들러로 등록
function addDelete() {
    //  현재글의 id
    const id = $("input[name='id']").val().trim();

    $("[data-cmtdel-id]").click(function () {
        if (!confirm("삭제하겟나 닝겐")) return;
